...
DG 的优点很明显, 但其仍有不少难以避免的缺点, 其中最主要的是其自由度多, 这对求解最后代数方程组带来很大困难, 以及其双线性形式的强制性依赖于充分大的惩罚参数. 2009 年, Cockburn 等人针对二阶椭圆边值问题提出了杂交有限元方法的一致框架, 其中杂交间断 Galerkin (hybrid discontinuous Galerkin, HDG) 方法保持了 DG 方法的优点, 还克服了 DG 方法的缺点. 虽然大多数文献关注于标准网格, 但在近来, HDG 方法的标准版本自然扩展到更一般的多面体网格. 同样适用于一般多边形网格的高阶数值方法还有: 弱 Galerkin (weak Galerkin, WG) 方法, 虚单元法 (virtual element method, VEM), 杂交高阶 (hybrid high-order, HHO) 方法等. 类似于经典混合有限元, HDG 中的另一个经常性的研究课题是研究变量的超逼近性. 这一点, HHO 方法带来了两个重要的概念性进展. 首先, 在方法的公式中加入了局部的重构, 而不是用于后处理; 其次, 在局部建立了更高阶的稳定项. 因此, HHO 方法中变量的超逼近是内在的. 



接下来我们将注意力放在 HHO 方法求解一类拟牛顿流体问题. 拟牛顿流体是对复杂流体 (非牛顿流体) 的一种简单近似. 复杂流体是指黏性系数是非恒定的一类流体, 这类流体在现实生活中广泛存在, 例如水泥, 牙膏, 血液等糊状物或聚合物溶液都是复杂流体. 实验测量表明, 粘度随形变速率的变化而变化. 描述复杂流体最简单的方法是绘制粘度测量值与外加剪切速率的关系图, 然后用简单的模板粘度函数拟合得到的曲线, 调整一些参数. 这是准牛顿流体模型的主要思想。准牛顿流体可以看作是复杂流体世界中的第一步：它们可以被认为是更复杂的粘塑性或粘弹性流体的简单近似。
